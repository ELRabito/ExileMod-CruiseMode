...
if (_keyCode in (actionKeys "User8")) exitWith //You can change the Custom User action to a different one (Keep in mind Exile already uses certain ones (1-4)).
{
	...
	{
		if (((assignedVehicleRole player) #0) isEqualTo "driver") then
		{
			if((getCruiseControl vehicle player) #1 == true) then {vehicle player setCruiseControl [0, false]}else{vehicle player setCruiseControl [speed player, true]};
		};
	};
	true
};
			...
			else 
			{
				...
				if (primaryWeapon player != "") then
				{
					if (primaryWeapon player != currentWeapon player) then
					{
						...
						if(ExileClientMWState isEqualTo [] or {primaryWeapon player != (ExileClientMWState #0)})	then 
						{
							player selectWeapon (primaryWeapon player);
						}
						else
						{
							player selectWeapon [(primaryWeapon player), ExileClientMWState #1, ExileClientMWState #2];
						};
					};
				};
				/* Holster+ End*/
			...
			else 
			{
				if (handgunWeapon player != "") then
				{
					...
					if (handgunWeapon player != currentWeapon player) then
					{
						...
						if(currentWeapon player == secondaryWeapon player) then {ExileClientSWState = weaponState player};
						if(ExileClientHGState isEqualTo [] or {handgunWeapon player != (ExileClientHGState #0)}) then 
						{	
							player selectWeapon (handgunWeapon player);
						}
						else
						{
							player selectWeapon [(handgunWeapon player), ExileClientHGState #1, ExileClientHGState #2];
						};
					};
					/* Holster+ End*/
				};
			...
			else 
			{
				if (secondaryWeapon player != "") then
				{
					...
					if (secondaryWeapon player != currentWeapon player) then
					{
						if(currentWeapon player == primaryWeapon player) then {ExileClientMWState = weaponState player};
						if(currentWeapon player == handgunWeapon player) then {ExileClientHGState = weaponState player};
						if(ExileClientSWState isEqualTo [] or {secondaryWeapon player != (ExileClientSWState #0)}) then 
						{
							player selectWeapon (secondaryWeapon player);
						}
						else
						{
							player selectWeapon [secondaryWeapon player, ExileClientSWState #1, ExileClientSWState #2];
						};
					};
					/* Holster+ End*/
				};
	...
	{ 
		if !(ExileClientIsHandcuffed || ExileIsPlayingRussianRoulette) then 
		{
			...
			else
			{
				...
				{
					if (ExileClientPlayerHolsteredWeapon isNotEqualTo []) then
					{
						player selectWeapon [ExileClientPlayerHolsteredWeapon #0, ExileClientPlayerHolsteredWeapon #1, ExileClientPlayerHolsteredWeapon #2];
						if(currentWeapon player == primaryWeapon player) then {ExileClientMWState = weaponState player};
						if(currentWeapon player == handgunWeapon player) then {ExileClientHGState = weaponState player};
						if(currentWeapon player == secondaryWeapon player) then {ExileClientSWState = weaponState player};
					};
